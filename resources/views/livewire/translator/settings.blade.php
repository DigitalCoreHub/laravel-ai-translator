<div class="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
    <div class="mx-auto max-w-5xl px-6 py-10 space-y-8">
        ...
        <section class="space-y-4">
            @forelse (($providers ?? []) as $name => $config)
                <div class="rounded-2xl border border-slate-200 bg-white/70 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
                    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            @if (!empty($status[$name]['message'] ?? null))
                    ...
                    <dl class="mt-4 grid gap-3 md:grid-cols-2">
                        ...
                        @empty
                            <div class="rounded-xl border border-dashed border-slate-300 bg-white/60 px-4 py-3 text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-400">ENV değeri bulunamadı.</div>
                        @endforelse
                    </dl>
                </div>
            @empty
                <p class="rounded-2xl border border-dashed border-slate-300 bg-white/60 p-6 text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-400">Provider yapılandırması bulunamadı.</p>
            @endforelse
        </section>
    </div>
</div>
